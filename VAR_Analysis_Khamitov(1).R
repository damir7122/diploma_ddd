...
BZF        <- getSymbols("BZ=F", src = "yahoo", from = "2013-01-01", to = "2021-12-31",
                          auto.assign = FALSE)
...
oil_price <- na.locf(oil_price, fromLast = TRUE, na.rm = FALSE)
...
dual_axis_plot <- function(data, title) {
  dates <- index(data)
  oil   <- as.numeric(data$Oil)
  icln  <- as.numeric(data$ICLN)

  plot(dates, oil,
       type = "l", col = "#1f4e79", lwd = 1.2,
       main = title, xlab = "", ylab = "Oil (USD)",
       xaxt = "n")                              # ось X нарисуем вручную

  # подписи по оси X
  axis.Date(1, at = pretty(dates), format = "%Y-%m")

  par(new = TRUE)
  plot(dates, icln,
       type = "l", col = "#c45911", lwd = 1.2,
       axes = FALSE, xlab = "", ylab = "")

  axis(4, col.axis = "#c45911")
  mtext("ICLN (USD)", side = 4, line = 2.5, col = "#c45911")
...
dual_axis_plot(data1, "Oil & ICLN (2013-2017)")
dual_axis_plot(data2, "Oil & ICLN (2019-2021)")
